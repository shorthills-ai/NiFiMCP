...
@app.get("/auth-test")
async def auth_test(current_user: dict = Depends(verify_token)):
...
async def read_users_me(current_user: dict = Depends(verify_token)):
    ...
    return {"valid": current_user["valid"], "token_expires": current_user["exp"]}


@app.post("/test-manual-auth")
async def test_manual_auth(request: Request):
    """Test endpoint to verify manual token authentication"""
    try:
        logger.info("Testing manual token authentication")
        ...
        return {
            "message": "Manual JWT authentication successful",
            ...
            "authentication_flow": [
                ...
                "4. Continue with protected operation"
            ]
        }
        
    except HTTPException as e:
        logger.error(f"Manual auth test failed: {e.detail}")
        raise e
    except Exception as e:
        logger.error(f"Unexpected error in manual auth test: {str(e)}")
        raise HTTPException(status_code=500, detail="Internal authentication error")


@app.get("/auth-info")
async def auth_info():
    """Information about the JWT authentication system"""
    return {
        "authentication_system": "JWT Secret Key with Optional Admin Auth",
        "description": "This system provides multiple token generation methods",
        "token_generation_endpoints": {
            "/token": {
                "method": "POST",
                "requires_credentials": True,
                "credentials": "admin username and password",
                "description": "Generate JWT token - requires admin authentication"
            },
            "/create_access_token": {
                "method": "POST",
                "requires_credentials": False,
                "description": "Create access token directly - no authentication required"
            }
        },
        "token_validation": {
            "description": "All tokens are validated using the same JWT secret key",
            "header_format": "Authorization: Bearer <token>",
            "expiration": f"{JWT_EXPIRATION_HOURS} hours"
        },
        "security_features": [
            "Tokens created with JWT_SECRET_KEY only",
            "Tokens expire after JWT_EXPIRATION_HOURS",
            "/token endpoint requires admin credentials",
            "/create_access_token endpoint is direct generation",
            "No persistent user sessions required"
        ]
    }
