...
app.mount("/static", StaticFiles(directory="static_downloads"), name="static")
...
def clean_extracted_text(raw_text: str) -> str:
    """
    Cleans raw text extracted by pdftotext to remove excessive spaces.
    """
    # Collapse all whitespace runs
    text = " ".join(raw_text.split())
    # Fix classic 'W i t h' -> 'With' style artifacts
...
def convert_pptx_to_pdf(input_pptx_path: str, output_dir: str) -> str:
    ...
    if len(new_pdfs) > 1:
        logger.warning(f"Multiple PDFs found after conversion. Using first: {new_pdfs}")
...
def extract_text_from_pptx(
    ...
    """
    End-to-end PPTX -> PDF -> per-slide PDFs -> text extraction -> chunking.
    Returns list of chunk dicts with 'content', 'slide_number', and 'slide_url'.
    """
    chunks = []
    max_chunk_size = 1028
    min_chunk_size = 100
    current_chunk = []
    current_size = 0
    first_slide_in_chunk = None
    first_slide_url = None

    # Step 1: Create session folder for this request
    session_id = str(uuid.uuid4())
    session_folder = os.path.join("static_downloads", session_id)
    os.makedirs(session_folder, exist_ok=True)
    logger.info(f"Created session folder: {session_folder}")

    with tempfile.TemporaryDirectory() as tmpdir:
        # Step 2: Save uploaded PPTX to temp file
        input_pptx_path = os.path.join(tmpdir, original_filename)
        with open(input_pptx_path, "wb") as f:
            f.write(file_bytes.getvalue())

        # Step 3: Convert PPTX to single PDF
        try:
        ...
        logger.info(f"Generated full PDF: {full_pdf_path}")

        # Step 4: Split single PDF into per-slide PDFs
        ...
        # Step 5: Find all generated slide PDFs
        slide_pdfs = sorted([
            f for f in os.listdir(tmpdir)
            if re.match(r"slide_\d+\.pdf", f)
        ])

        if not slide_pdfs:
            raise Exception("No slide PDFs generated after splitting.")

        # Step 6: Process each slide PDF
        for idx, pdf_name in enumerate(slide_pdfs):
            slide_num_match = re.search(r'slide_(\d+)\.pdf', pdf_name)
            slide_number = int(slide_num_match.group(1)) if slide_num_match else idx + 1

            # Move slide PDF to static folder
            src_pdf = os.path.join(tmpdir, pdf_name)
            dst_pdf = os.path.join(session_folder, pdf_name)
            shutil.copy(src_pdf, dst_pdf)

            # Build serving URL
            this_slide_url = f"{server_base_url}/static/{session_id}/{pdf_name}"

            # Extract text from slide PDF using pdftotext
            extracted_text_file = os.path.join(tmpdir, f"{pdf_name}.txt")
            try:
                subprocess.run(
                    ["pdftotext", src_pdf, extracted_text_file],
                    check=True
                )
            ...
            # Build final slide text with header
            slide_texts = [f"[Slide {slide_number}]", slide_text]
            slide_content = " ".join(slide_texts)
            slide_size = len(slide_content)

            # Start new chunk if necessary
            if first_slide_in_chunk is None:
                first_slide_in_chunk = slide_number
                first_slide_url = this_slide_url
            ...
            if current_size + slide_size > max_chunk_size and current_chunk:
                ...
                if len(chunk_text) >= min_chunk_size:
                    chunks.append({
                        "content": chunk_text,
                        "slide_number": first_slide_in_chunk,
                        "slide_url": first_slide_url
                    })
                current_chunk = []
                current_size = 0
                first_slide_in_chunk = slide_number
            ...
            # Add this slide's content
            current_chunk.append(slide_content)
            current_size += slide_size

            # If chunk got too big after add, flush immediately
        ...
        if current_chunk:
            ...
            if len(chunk_text) >= min_chunk_size:
                chunks.append({
                    "content": chunk_text,
                    "slide_number": first_slide_in_chunk or slide_number,
                    "slide_url": first_slide_url
                })
...
def is_valid_office_file(file_bytes: BytesIO) -> bool:
    file_bytes.seek(0)
    return zipfile.is_zipfile(file_bytes)
...
async def extract_text(request: Request, file: UploadFile = File(...)):
    try:
        ...
        if filename_lower.endswith(".docx"):
            ...
            server_base_url = f"{request.url.scheme}://{request.headers.get('host')}"
            chunks = extract_text_from_pptx(file_bytes, web_url, server_base_url, filename)
            file_type = "pptx"
        ...
        for idx, chunk in enumerate(chunks):
            # Check if chunk is a dict (pptx) or plain string (docx)
            ...
            # Add slide_number and slide_url if present
...
async def upload_to_weaviate(req: UploadRequest, authorization: str = Header(None)):
    try:
        ...
        logger.info("Connected to Weaviate and accessed 'Chunks' collection")
...
Only answer based on the provided context. Do not hallucinate or guess.
        ...
        try:
            llm_response = await generate_completion(prompt, system_prompt);print(llm_response)
            if not llm_response:
